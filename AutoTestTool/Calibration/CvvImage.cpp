...
void CvvImage::Show( HDC dc, int x, int y, int w, int h, int from_x, int from_y )
{
    if( m_img && m_img->depth == IPL_DEPTH_8U )
    {
        ...
        SetDIBitsToDevice(dc, x, y, sw, sh, from_x, from_y, from_y, sh,
            m_img->imageData + from_y*m_img->widthStep, bmi, DIB_RGB_COLORS );
    }
}
...
void CvvImage::DrawToHDC( HDC hDCDst, RECT* pDstRect )
{
    if( pDstRect && m_img && m_img->depth == IPL_DEPTH_8U && m_img->imageData )
    {
        ...
        {
            SetStretchBltMode(hDCDst, COLORONCOLOR);       // handle to device context
        }
        FillBitmapInfo( bmi, bmp_w, bmp_h, Bpp(), m_img->origin );
...
void  CvvImage::ShowEx( HDC dc, int x, int y, int w, int h, int from_x, int from_y, bool bCenter)
{
    if( m_img && m_img->depth == IPL_DEPTH_8U )
    {
        uchar buffer[sizeof(BITMAPINFOHEADER) + 1024];
        ...
        int sw = MAX( MIN( bmp_w - from_x, w ), 0 );
        int sh = MAX( MIN( bmp_h - from_y, h ), 0 );
        if( w < bmp_w || h < bmp_h )            // 需要进行缩小
        {
            if(w*bmp_h>h*bmp_w){                // 宽需要进行缩小
                sw = sh*bmp_w/bmp_h;
            }else{
                sh = sw*bmp_h/bmp_w;
            }
        ...
        bmi, DIB_RGB_COLORS );*/
        ::StretchDIBits(dc, x, y, sw, sh, from_x, from_y, bmp_w - from_x, bmp_h - from_y,
